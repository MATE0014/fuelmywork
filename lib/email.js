...
function getEmailProviderErrorMessage(error) {
  return error?.response?.body?.message || error?.response?.body?.error || error?.message || "Unknown email provider error"
}

export async function sendOTPEmail(email, otp, name, purpose = "verification") {
  try {
    const sendSmtpEmail = new SendSmtpEmail()

    if (purpose === "account deletion") {
      sendSmtpEmail.subject = "Confirm Account Deletion - Fuelmywork"
      sendSmtpEmail.htmlContent = `
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #dc3545 0%, #c82333 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Confirm Account Deletion</h1>
          </div>
          ...
          <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
            ...
            <p style="font-size: 16px; margin-bottom: 25px;">
              We received a request to permanently delete your Fuelmywork account. To confirm this action, please use the verification code below:
            </p>
            
            <div style="background: white; border: 2px dashed #dc3545; border-radius: 8px; padding: 20px; text-align: center; margin: 25px 0;">
              <h3 style="color: #dc3545; margin: 0 0 10px 0;">Your Deletion Confirmation Code</h3>
            ...
            <div style="background: #f8d7da; border: 1px solid #f5c6cb; border-radius: 5px; padding: 15px; margin: 20px 0;">
              <p style="margin: 0; font-size: 14px; color: #721c24;">
                <strong>Warning:</strong> This action is permanent and cannot be undone. All your data, including profile information, payments, and supporter relationships will be permanently deleted.
              </p>
            </div>
            
            <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px; padding: 15px; margin: 20px 0;">
              <p style="margin: 0; font-size: 14px; color: #856404;">
                <strong>Security Note:</strong> If you didn't request account deletion, please ignore this email and consider changing your password.
              </p>
            </div>
      ...
      // Default verification email content
    ...
    return { success: false, error: getEmailProviderErrorMessage(error) }
  }
}
    ...
    sendSmtpEmail.htmlContent = `
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
          ...
          <div style="background: #f8d7da; border: 1px solid #f5c6cb; border-radius: 5px; padding: 15px; margin: 20px 0;">
            <p style="margin: 0; font-size: 14px; color: #721c24;">
              <strong>Security Alert:</strong> If you didn't request a password reset, please ignore this email. Your password will remain unchanged. Consider changing your password if you suspect any unauthorized activity.
            </p>
          </div>
